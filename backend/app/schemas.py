...
class ExplanationSelection(BaseModel):
    """The provider can select facts, but cannot author displayed text or numbers."""

    model_config = ConfigDict(extra="forbid", strict=True)

    summary: list[str] = Field(min_length=1, max_length=3)
    strengths: list[str] = Field(min_length=1, max_length=8)
    risks: list[str] = Field(min_length=1, max_length=4)
    tradeoffs: list[str] = Field(min_length=1, max_length=6)
